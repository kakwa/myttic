#!/bin/bash
export PATH=$PATH:/home/kakwa/ns-allinone-2.34/bin
t=`ls`
tq=40
for i in $t
	do
	if [ -d $i ]; then
	cd $i
	sed -i "s/ns queue-limit \$nred \$ndest.*/ns queue-limit \$nred \$ndest 300/" simul1.tcl
	sed -i "s/q0n1 configQ 0 0.*/q0n1 configQ 0 0 290 290 1/" simul1.tcl
	sed -i "s/q0n1 configQ 1 0.*/q0n1 configQ 1 0 $tq 290 1/" simul1.tcl
	        sed -i "s/exp set burst_time_.*/exp set burst_time_ 1ms/" simul1.tcl
		        sed -i "s/exp set idle_time_.*/exp set idle_time_ 2ms/" simul1.tcl
			        sed -i "s/exp set rate_.*/exp set rate_ 90Mb/" simul1.tcl
	tq=$(( $tq +50 ))
	cd ..
	fi
	done

